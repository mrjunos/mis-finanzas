...
export default function Transactions({ currentContext }) {
    ...
    // Daily activity chart data: ingresos vs egresos per day (excludes transfers)
    const dailyChartData = useMemo(() => {
        const activeCurrency = selectedCurrency || (chartData.currencies.length > 0 ? chartData.currencies[0] : 'USD');
        const dayMap = {};

        [...processedTransactions]
            .filter(t => (t.currency || 'USD') === activeCurrency && !(t.type === 'transfer' || t.isTransfer))
            .reverse()
            .forEach(t => {
                const dayKey = format(t.date, 'yyyy-MM-dd');
                const dayLabel = format(t.date, 'dd MMM', { locale: es });
                if (!dayMap[dayKey]) {
                    dayMap[dayKey] = { date: dayLabel, ingresos: 0, egresos: 0 };
                }
                if (t.type === 'credit') {
                    dayMap[dayKey].ingresos += Number(t.amount);
                } else {
                    dayMap[dayKey].egresos += Number(t.amount);
                }
            });
                    ...
                    <div className="flex flex-col gap-4 mt-2">
                        ...
                        {dailyChartData.length > 0 && (
                            <div className="flex flex-col gap-2 mt-2">
                                ...
                                <div className="w-full bg-white/40 rounded-2xl border border-white/40 shadow-sm p-4 backdrop-blur-md">
                                    <div className="h-72 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={dailyChartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                                                <XAxis dataKey="date" stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} />
                                                <YAxis stroke="#94a3b8" fontSize={12} axisLine={false} tickLine={false} tickFormatter={(val) => formatCompactNumber(val)} />
                                                <Tooltip
                                                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                                                    formatter={(value, name) => [formatCurrency(value, searchSummary.currency), name === 'ingresos' ? 'Ingresos' : 'Egresos']}
                                                    labelStyle={{ color: '#475569', fontWeight: '500', marginBottom: '8px' }}
                                                />
